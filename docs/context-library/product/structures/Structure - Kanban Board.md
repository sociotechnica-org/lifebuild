...
- Director opens Gold project "Home Renovation" → Kanban shows: To Do (5 tasks), In Progress (2 tasks), Done (3 tasks) → director drags "Get contractor quotes" from To Do to In Progress → WIP indicator shows 3/3 → visual signal that focus limit is reached, but not locked.
- Bronze task generated by "Morning Routine" system appears in project Kanban → marked with system-generated indicator → director completes it → task moves to Done → Bronze stack on The Table updates simultaneously.

### Anti-Examples

- **Enforcing WIP limit as a hard block that prevents dragging additional tasks to In Progress** — the WIP limit recommends, not restricts. A visual indicator signals "you're at capacity," but the director can override. Soft guidance, not hard control.
