...
- Language shapes whether builders avoid planning or embrace it
...
- Scenario: Builder planned 3 Gold tasks for the week and completed 2. The weekly review surfaces.
...
- Scenario: Builder realizes Wednesday that their Silver project needs to be swapped for a more urgent one.
- Input: Builder drags a new project into the Silver slot, replacing the current one.
- Correct output: The swap happens immediately with no "reason for change" dialog. The UI feels like adjusting a strategy board. The change is logged silently for calibration data. Marvin does not ask why the change was made.
