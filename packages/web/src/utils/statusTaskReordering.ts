 ...
 * @param tasksInTargetStatus - All tasks currently in the target status (sorted by position)
...
export function calculateStatusTaskReorder(
  ...
  if (otherTasks.length === 0) {
    // Empty status - use position 1000 as starting point
    newPosition = 1000
  ...
  if (needsNormalization) {
    // Normalize all positions in the status with even spacing
    const normalizedResults = normalizeStatusPositions(
      draggedTask,
      targetStatus,
      targetIndex,
      otherTasks,
...
/**
 * Normalizes all positions in a status to have even spacing
 */
...
/**
 * Simplified drop target calculation for drag and drop with status-based tasks
 * Returns the target status and index where the task should be inserted
 */
export function calculateStatusDropTarget(
  ...
  // If dropping on a task in the same status and below the dragged task,
