...
export const WorkspaceProvider: React.FC<WorkspaceProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!isInitialized) {
      ...
      // Determine current workspace - prioritize URL parameter for deep links
      const urlParams = new URLSearchParams(window.location.search)
      const urlWorkspaceId = urlParams.get('storeId')
      const storedWorkspaceId = localStorage.getItem(WORKSPACE_STORAGE_KEY)

      // Check if URL workspace ID is valid
      const isValidUrlWorkspace =
        urlWorkspaceId && user.instances.some(w => w.id === urlWorkspaceId)

      // Check if stored workspace ID is valid
      const isValidStoredWorkspace =
        storedWorkspaceId && user.instances.some(w => w.id === storedWorkspaceId)

      let selectedWorkspaceId: string | null = null

      if (isValidUrlWorkspace) {
        // Prioritize URL parameter for deep links
        selectedWorkspaceId = urlWorkspaceId
      } else if (isValidStoredWorkspace) {
        // Fall back to stored workspace
        ...
        selectedWorkspaceId = defaultWorkspace?.id || user.instances[0]?.id || null
      }
  ...
  const switchWorkspace = useCallback(
    ...
    [workspaces, makeAuthenticatedRequest, refreshWorkspaces]
  )
  ...
  const createWorkspace = useCallback(
    ...
    [makeAuthenticatedRequest, refreshWorkspaces]
  )
  ...
  const deleteWorkspace = useCallback(
    ...
    [currentWorkspaceId, makeAuthenticatedRequest, refreshWorkspaces]
  )
