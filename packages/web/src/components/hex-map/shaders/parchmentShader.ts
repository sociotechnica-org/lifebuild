import * as THREE from 'three'

const vertexShader = `
  varying vec3 vWorldPos;
  varying vec3 vLocalPos;

  void main() {
    vLocalPos = position;
    vec4 worldPosition = modelMatrix * vec4(position, 1.0);
    vWorldPos = worldPosition.xyz;
    gl_Position = projectionMatrix * viewMatrix * worldPosition;
  }
`

const fragmentShader = `
  precision highp float;

  uniform vec3 uHighlight;
  uniform float uHighlightStrength;

  // Noise scales
  uniform float uRegionScale;
  uniform float uRegion2Scale;
  uniform float uBlotchScale;
  uniform float uBlotch2Scale;
  uniform float uWetEdgeScale;
  uniform float uGrainScale;
  uniform float uFiberScaleX;
  uniform float uFiberScaleY;
  uniform float uSpeckleScale;

  // Strengths / intensities
  uniform float uSageStrength;
  uniform float uStainStrength;
  uniform float uPoolLightStrength;
  uniform float uPoolDarkStrength;
  uniform float uWetEdgeStrength;
  uniform float uGrainIntensity;
  uniform float uFiberIntensity;
  uniform float uSpeckleIntensity;

  // Hex edge
  uniform float uEdgeStrength;
  uniform float uEdgeWidth;
  uniform float uNoiseSeed;

  varying vec3 vWorldPos;
  varying vec3 vLocalPos;

  //
  // Simplex 3D noise — Ashima Arts (MIT License)
  // https://github.com/ashima/webgl-noise
  //
  vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
  vec4 mod289(vec4 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
  vec4 permute(vec4 x) { return mod289(((x * 34.0) + 10.0) * x); }
  vec4 taylorInvSqrt(vec4 r) { return 1.79284291400159 - 0.85373472095314 * r; }

  float snoise(vec3 v) {
    const vec2 C = vec2(1.0 / 6.0, 1.0 / 3.0);
    const vec4 D = vec4(0.0, 0.5, 1.0, 2.0);

    vec3 i  = floor(v + dot(v, C.yyy));
    vec3 x0 = v - i + dot(i, C.xxx);

    vec3 g = step(x0.yzx, x0.xyz);
    vec3 l = 1.0 - g;
    vec3 i1 = min(g.xyz, l.zxy);
    vec3 i2 = max(g.xyz, l.zxy);

    vec3 x1 = x0 - i1 + C.xxx;
    vec3 x2 = x0 - i2 + C.yyy;
    vec3 x3 = x0 - D.yyy;

    i = mod289(i);
    vec4 p = permute(permute(permute(
              i.z + vec4(0.0, i1.z, i2.z, 1.0))
            + i.y + vec4(0.0, i1.y, i2.y, 1.0))
            + i.x + vec4(0.0, i1.x, i2.x, 1.0));

    float n_ = 0.142857142857;
    vec3 ns = n_ * D.wyz - D.xzx;

    vec4 j = p - 49.0 * floor(p * ns.z * ns.z);

    vec4 x_ = floor(j * ns.z);
    vec4 y_ = floor(j - 7.0 * x_);

    vec4 x = x_ * ns.x + ns.yyyy;
    vec4 y = y_ * ns.x + ns.yyyy;
    vec4 h = 1.0 - abs(x) - abs(y);

    vec4 b0 = vec4(x.xy, y.xy);
    vec4 b1 = vec4(x.zw, y.zw);

    vec4 s0 = floor(b0) * 2.0 + 1.0;
    vec4 s1 = floor(b1) * 2.0 + 1.0;
    vec4 sh = -step(h, vec4(0.0));

    vec4 a0 = b0.xzyw + s0.xzyw * sh.xxyy;
    vec4 a1 = b1.xzyw + s1.xzyw * sh.zzww;

    vec3 p0 = vec3(a0.xy, h.x);
    vec3 p1 = vec3(a0.zw, h.y);
    vec3 p2 = vec3(a1.xy, h.z);
    vec3 p3 = vec3(a1.zw, h.w);

    vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2,p2), dot(p3,p3)));
    p0 *= norm.x;
    p1 *= norm.y;
    p2 *= norm.z;
    p3 *= norm.w;

    vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);
    m = m * m;
    return 42.0 * dot(m * m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3)));
  }

  float hexEdgeDist(vec2 p, float R) {
    float inR = R * 0.8660254;
    vec2 q = abs(p);
    float d = max(q.x, 0.5 * q.x + 0.8660254 * q.y);
    return inR - d;
  }

  const vec3 PARCHMENT       = vec3(0.78, 0.68, 0.50);
  const vec3 PARCHMENT_LIGHT = vec3(0.87, 0.81, 0.67);
  const vec3 PARCHMENT_DARK  = vec3(0.68, 0.58, 0.41);
  const vec3 SAGE_WASH       = vec3(0.65, 0.71, 0.52);
  const vec3 WARM_STAIN      = vec3(0.73, 0.58, 0.40);

  void main() {
    vec2 wp = vWorldPos.xz;
    vec2 seededWp = wp + vec2(uNoiseSeed * 17.371, -uNoiseSeed * 11.913);

    float region = snoise(vec3(seededWp * uRegionScale, 0.0 + uNoiseSeed * 3.1)) * 0.5 + 0.5;
    float region2 = snoise(vec3(seededWp * uRegion2Scale, 2.1 + uNoiseSeed * 5.7)) * 0.5 + 0.5;

    float blotch = snoise(vec3(seededWp * uBlotchScale, 3.7 + uNoiseSeed * 8.3)) * 0.5 + 0.5;
    float blotch2 = snoise(vec3(seededWp * uBlotch2Scale, 5.2 + uNoiseSeed * 10.1)) * 0.5 + 0.5;

    float wetEdge = abs(snoise(vec3(seededWp * uWetEdgeScale, 8.5 + uNoiseSeed * 13.7)));
    float grain = snoise(vec3(seededWp * uGrainScale, 7.3 + uNoiseSeed * 2.9));
    float fiber = snoise(vec3(
      seededWp.x * uFiberScaleX,
      seededWp.y * uFiberScaleY,
      11.0 + uNoiseSeed * 4.3
    ));
    float speckle = snoise(vec3(seededWp * uSpeckleScale, 19.1 + uNoiseSeed * 6.5));

    vec3 color = PARCHMENT;

    float sageMask = smoothstep(0.45, 0.70, region) * smoothstep(0.35, 0.60, region2);
    color = mix(color, SAGE_WASH, sageMask * uSageStrength);

    float stainMask = smoothstep(0.30, 0.55, 1.0 - region) * smoothstep(0.25, 0.55, 1.0 - region2);
    color = mix(color, WARM_STAIN, stainMask * uStainStrength);

    float pooling = blotch * 0.55 + blotch2 * 0.45;
    color = mix(color, PARCHMENT_LIGHT, smoothstep(0.50, 0.80, pooling) * uPoolLightStrength);
    color = mix(color, PARCHMENT_DARK, smoothstep(0.50, 0.80, 1.0 - pooling) * uPoolDarkStrength);

    float edgeDarken = smoothstep(0.02, 0.15, wetEdge) * (1.0 - smoothstep(0.15, 0.30, wetEdge));
    color *= 1.0 - edgeDarken * uWetEdgeStrength;

    color += grain * uGrainIntensity;
    color += fiber * uFiberIntensity;
    color += speckle * uSpeckleIntensity;

    float dist = hexEdgeDist(vLocalPos.xz, 1.0);
    float edgeFactor = 1.0 - smoothstep(0.0, uEdgeWidth, dist);
    color *= 1.0 - edgeFactor * uEdgeStrength;

    color = mix(color, uHighlight, uHighlightStrength);

    gl_FragColor = vec4(color, 1.0);
  }
`

export type ParchmentShaderParams = {
  regionScale: number
  region2Scale: number
  blotchScale: number
  blotch2Scale: number
  wetEdgeScale: number
  grainScale: number
  fiberScaleX: number
  fiberScaleY: number
  speckleScale: number
  sageStrength: number
  stainStrength: number
  poolLightStrength: number
  poolDarkStrength: number
  wetEdgeStrength: number
  grainIntensity: number
  fiberIntensity: number
  speckleIntensity: number
  edgeStrength: number
  edgeWidth: number
}

export const JESS_PREFERENCE_PARCHMENT_PARAMS: ParchmentShaderParams = {
  regionScale: 0.09,
  region2Scale: 0.25,
  blotchScale: 0.16,
  blotch2Scale: 0.2,
  wetEdgeScale: 0.0855,
  grainScale: 6.3,
  fiberScaleX: 6.74,
  fiberScaleY: 3.3,
  speckleScale: 7,
  sageStrength: 0.44,
  stainStrength: 0.42,
  poolLightStrength: 0.186,
  poolDarkStrength: 0.305,
  wetEdgeStrength: 0.003,
  grainIntensity: 0.0222,
  fiberIntensity: 0.0256,
  speckleIntensity: 0.0288,
  edgeStrength: 0.11,
  edgeWidth: 0.04,
}

type UniformValue = number | THREE.Vector3

const setUniform = (
  uniforms: Record<string, { value: UniformValue }>,
  key: string,
  value: UniformValue
) => {
  uniforms[key]!.value = value
}

export const applyParchmentUniforms = (
  material: THREE.ShaderMaterial,
  params: ParchmentShaderParams,
  options?: {
    seed?: number
    edgeStrength?: number
    edgeWidth?: number
    highlightColor?: THREE.ColorRepresentation
    highlightStrength?: number
  }
) => {
  const uniforms = material.uniforms as Record<string, { value: UniformValue }>

  setUniform(uniforms, 'uRegionScale', params.regionScale)
  setUniform(uniforms, 'uRegion2Scale', params.region2Scale)
  setUniform(uniforms, 'uBlotchScale', params.blotchScale)
  setUniform(uniforms, 'uBlotch2Scale', params.blotch2Scale)
  setUniform(uniforms, 'uWetEdgeScale', params.wetEdgeScale)
  setUniform(uniforms, 'uGrainScale', params.grainScale)
  setUniform(uniforms, 'uFiberScaleX', params.fiberScaleX)
  setUniform(uniforms, 'uFiberScaleY', params.fiberScaleY)
  setUniform(uniforms, 'uSpeckleScale', params.speckleScale)

  setUniform(uniforms, 'uSageStrength', params.sageStrength)
  setUniform(uniforms, 'uStainStrength', params.stainStrength)
  setUniform(uniforms, 'uPoolLightStrength', params.poolLightStrength)
  setUniform(uniforms, 'uPoolDarkStrength', params.poolDarkStrength)
  setUniform(uniforms, 'uWetEdgeStrength', params.wetEdgeStrength)
  setUniform(uniforms, 'uGrainIntensity', params.grainIntensity)
  setUniform(uniforms, 'uFiberIntensity', params.fiberIntensity)
  setUniform(uniforms, 'uSpeckleIntensity', params.speckleIntensity)

  setUniform(uniforms, 'uEdgeStrength', options?.edgeStrength ?? params.edgeStrength)
  setUniform(uniforms, 'uEdgeWidth', options?.edgeWidth ?? params.edgeWidth)
  setUniform(uniforms, 'uNoiseSeed', options?.seed ?? 0)
  setUniform(uniforms, 'uHighlightStrength', options?.highlightStrength ?? 0)

  const highlight = new THREE.Color(options?.highlightColor ?? '#000000')
  ;(uniforms.uHighlight!.value as THREE.Vector3).set(highlight.r, highlight.g, highlight.b)
}

export const createParchmentMaterial = (
  params: ParchmentShaderParams = JESS_PREFERENCE_PARCHMENT_PARAMS
): THREE.ShaderMaterial => {
  const material = new THREE.ShaderMaterial({
    uniforms: {
      uHighlight: { value: new THREE.Vector3(0, 0, 0) },
      uHighlightStrength: { value: 0.0 },

      uRegionScale: { value: params.regionScale },
      uRegion2Scale: { value: params.region2Scale },
      uBlotchScale: { value: params.blotchScale },
      uBlotch2Scale: { value: params.blotch2Scale },
      uWetEdgeScale: { value: params.wetEdgeScale },
      uGrainScale: { value: params.grainScale },
      uFiberScaleX: { value: params.fiberScaleX },
      uFiberScaleY: { value: params.fiberScaleY },
      uSpeckleScale: { value: params.speckleScale },

      uSageStrength: { value: params.sageStrength },
      uStainStrength: { value: params.stainStrength },
      uPoolLightStrength: { value: params.poolLightStrength },
      uPoolDarkStrength: { value: params.poolDarkStrength },
      uWetEdgeStrength: { value: params.wetEdgeStrength },
      uGrainIntensity: { value: params.grainIntensity },
      uFiberIntensity: { value: params.fiberIntensity },
      uSpeckleIntensity: { value: params.speckleIntensity },

      uEdgeStrength: { value: params.edgeStrength },
      uEdgeWidth: { value: params.edgeWidth },
      uNoiseSeed: { value: 0 },
    },
    vertexShader,
    fragmentShader,
    polygonOffset: true,
    polygonOffsetFactor: 1,
    polygonOffsetUnits: 1,
  })

  applyParchmentUniforms(material, params)

  return material
}
