...
export const ChatInterface: React.FC = () => {
  ...
  // Listen for LLM response events to update processing state
  React.useEffect(() => {
    if (!selectedConversationId) return

    const messagesQuery = getConversationMessages$(selectedConversationId)
    const currentConversationId = selectedConversationId

    const unsubscribe = store.subscribe(messagesQuery, {
      onUpdate: async messages => {
        // Find llmResponseStarted events (these don't have role/message, they're special events)
        const recentUserMessages = messages.filter(m => m.role === 'user')
        const assistantResponses = messages.filter(
          m => m.role === 'assistant' && m.llmMetadata?.source === 'braintrust'
        )

        // Check if any recent user messages don't have final assistant responses yet
        const isProcessing = recentUserMessages.some(userMsg => {
          // Look for final braintrust responses (not tool-execution or error responses)
          const hasFinalResponse = assistantResponses.some(
            response =>
              response.responseToMessageId === userMsg.id &&
              response.llmMetadata?.source === 'braintrust' &&
              !response.llmMetadata?.toolCall
          )

          // If message is recent (within last 5 minutes) and no final response, consider it processing
          const isRecent = Date.now() - new Date(userMsg.createdAt).getTime() < 5 * 60 * 1000
          return isRecent && !hasFinalResponse
        })
    ...
    return () => {
      unsubscribe()
      // Clean up processing state when unmounting
      setProcessingConversations(prev => {
        const newSet = new Set(prev)
        newSet.delete(currentConversationId)
        return newSet
