  ...
  const createContactsBulk = async (parsedContacts: ParsedContact[]) => {
    ...
    for (const contact of parsedContacts) {
      try {
        ...
        // Check against already created contacts in this batch
        const alreadyCreated = results.created.find(c => c.email.toLowerCase() === contact.email.toLowerCase())
        if (alreadyCreated) {
          results.skipped.push({
            email: contact.email,
            name: contact.name,
        ...
        await store.commit(
          events.contactCreated({
            id: contactId,
            name: contact.name || contact.email.split('@')[0], // Use email prefix as default name
            email: contact.email,
            createdAt: new Date(),
          })
        )
