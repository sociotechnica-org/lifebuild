...
describe('EventProcessor Edge Cases', () => {
  ...
  describe('Store management edge cases', () => {
    ...
    it('should handle startMonitoring when store is stopping', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        `⚠️ Store ${storeId} is currently stopping, cannot start monitoring`
      )
  ...
  describe('Event processing edge cases', () => {
    ...
    it('should handle extremely long record content', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/📨.*x{50}\.\.\./))
  ...
  describe('Error handling and recovery', () => {
    ...
    it('should continue processing events even when one fails', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/📨.*Valid message/))
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/📨.*Another valid message/))
    ...
    it('should handle store stopping during event processing', () => {
      ...
      const records = [{ id: 'test', message: 'Test message', createdAt: new Date() }]
